...
- The upstream author recorded here (`Anthropic FSI`) does not match the
  `author` field in `.claude-plugin/plugin.json` (`Z.ai`). Attribution and the
  third-party provenance of these components must be settled before publishing.
