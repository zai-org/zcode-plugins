...
complete it. Do not run GitLab workflow commands until `glab` installation,
latest-version check, authentication, hostname, and active identity are
verified.

Use `glab` for all GitLab operations. Run `glab <command> --help` for detailed
flag information from the installed version. For experimental commands such as
`mr note`, `stack`, `mcp`, and `work-items`, always inspect the exact subcommand
help immediately before use because argument order and flags can change.

## Remote-change safety

- Before any write, state the resolved hostname, project, active username,
  target resource, and intended change.
- Require the user's explicit confirmation immediately before merge, delete,
  unprotect, pipeline cancel/retry, repository/group settings changes,
  secret/variable mutation, or token creation/revocation/rotation.
- Never retrieve, echo, log, or summarize secret values. For a secret or CI/CD
  variable, accept a local environment-variable name or file path and pass the
  value without placing it in chat or a command-line argument.
- Prefer a read-only preview and report the affected item count before bulk
  changes. Stop if the resolved host, project, target, or identity is ambiguous.
