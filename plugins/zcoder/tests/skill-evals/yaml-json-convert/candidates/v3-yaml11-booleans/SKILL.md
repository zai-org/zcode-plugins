...
2. Unquoted true/false become JSON true/false (lowercase — never True/False,
   never strings). **This includes the YAML 1.1 boolean words**: unquoted
   `on`, `off`, `yes`, `no` (and their capitalized forms) are booleans in
   YAML 1.1 — the dialect essentially all real-world YAML tooling implements
   (PyYAML, Symfony Config, compose files) — so they become JSON true/false
   too:
   - YAML `flags: [on, off]` → JSON `"flags": [true, false]` (CORRECT)
   - YAML `flags: [on, off]` → JSON `"flags": ["on", "off"]` (WRONG: keeps
     the raw spelling as strings)
   Only QUOTED `"on"` / `"yes"` stay strings.
